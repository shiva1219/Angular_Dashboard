...
export class WeeklyComponent implements ControlValueAccessor {
  ...
  @Input() public weeklyCaledar: any;
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (changes.weeklyCaledar && changes.weeklyCaledar.currentValue) {
      console.log("inputData weekly" + JSON.stringify(changes.weeklyCaledar));
      this.prepareWeeklyData(changes.weeklyCaledar.currentValue.days);
    }

  }

  prepareWeeklyData(days: any) {
    this.calArr = [];
    this.datesSelected = [];
    ...
    this.weeklyCalendarObj = this.calArr;
    this.weeklyCalendarChanged.emit(this.weeklyCalendarObj);
    console.log("weekly calendar obj--->" + JSON.stringify(this.weeklyCalendarObj));
  ...
  onDateSelection(event: any, date: NgbDateStruct) {
    ...
    if (this.disabled === false) {
      ...
      for (let i = 0; i < dates.length; i++) {
        console.log("dates--->" + JSON.stringify(dates[i]));
  ...
  prepareWeekObj(now, month1: any) {
    ...
    this.weeklyCalendarObj = byWeeklyArr;
    this.weeklyCalendarChanged.emit(this.weeklyCalendarChanged);
